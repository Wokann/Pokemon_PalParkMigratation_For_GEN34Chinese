...
int main(int argc, char *argv[]) {
    ...
    printf("%s's checksum are:\n", filename);
    printf("CRC32: %08x\n", crc);

    char md5Result[100], sha1Result[100];
    calculate_md5(inputfile, md5Result);
    calculate_sha1(inputfile, sha1Result);
...
int main(int argc, char *argv[]) {
    ...
    char *filename = strrchr(inputfile, '\\'); // 寻找最后一个反斜杠
    if (filename == NULL) {
        filename = inputfile; // 如果找不到反斜杠，则使用完整路径
    } else {
        filename++; // 否则跳过反斜杠
    }
    ...
    sprintf(command_md5, "certutil -hashfile \"%s\" MD5 > md5_output.txt", inputfile);
    sprintf(command_sha1, "certutil -hashfile \"%s\" SHA1 > sha1_output.txt", inputfile);
    system(command_md5);
    system(command_sha1);
    ...
    sprintf(outputFilename, "%s/%s_hash.txt", outputfile, filename);
