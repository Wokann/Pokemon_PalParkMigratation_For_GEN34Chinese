...
THUMB_FUNC BOOL ConvertRSStringToDPStringInternational(const u8 * rs_str, u16 * dp_str, u32 length, u32 language)
{
    ...
    for (i = 0; i < length - 1; i++)
    {
        ...
        case 0xEA: // RS: Opening double quote
            dp_str[j] = GetOpeningQuote(language);
            break;
        case 0xEB: // RS: Closing double quote
            dp_str[j] = GetClosingQuote(language);
            break;
        default:
            dp_str[j] = conversion_table[rs_str[i]][notFullWidth];
            break;
        }
        }
    }
    ...
    return TRUE;
            ...
            if(rs_str[i] >= 0x01 && rs_str[i] <= 0x05){
                dp_str[j] = r4 + 0x0101 - (rs_str[i] - 0x01) * 9;}
            else if(rs_str[i] >= 0x07 && rs_str[i] <= 0x10){
                dp_str[j] = r4 - 0x002C - (rs_str[i] - 0x07) * 9;}
            else if(rs_str[i] == 0x11)
            {
                i++;
                if(rs_str[i] <= 0x31)
                    dp_str[j] = r4 - 0x0086;
                else if(rs_str[i] >= 0x32 && rs_str[i] <= 0xF6)
                    dp_str[j] = r4 - 0x0081;
                i--;
            }
            else if(rs_str[i] >= 0x12 && rs_str[i] <= 0x1A){
                dp_str[j] = r4 - 0x008A - (rs_str[i] - 0x12) * 9;}
            else{//(rs_str[i] >= 0x1C && rs_str[i] <= 0x1E)
                dp_str[j] = r4 - 0x01DB - (rs_str[i] - 0x1C) * 9;} 
            ChineseCounts++;
            i++;*/
